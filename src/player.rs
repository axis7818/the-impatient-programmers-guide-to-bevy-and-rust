...
const MOVE_SPEED: f32 = 140.; // pixels per second
...
fn spawn_player(
    ...
    let layout = atlas_layouts.add(TextureAtlasLayout::from_grid(
        ...
        12,                 // at least 12 rows available (54?)
        None,
        None,
    ));
...
fn animate_player(
    time: Res<Time>,
    mut player: Single<(&mut AnimationState, &mut AnimationTimer, &mut Sprite), With<Player>>,
) {
    let (anim, timer, sprite) = &mut *player;
